...
class Py_Stack(interfaces.plugins.PluginInterface):
    ...
    def detect_python_version(self, task):
        ...
        if version:
           ...
           print("Could not detect Python version from process VADs")
        return version
    ...
    def _prepare(self, task, python_table_name):
        ...
        if self._version_key == (3, 6):
           head = pi._resolve_interp_head_36(task)
        else:
    ...
    def _load_symbol_table(self, version):
        from volatility3.plugins.windows.py_interpreter import SYMBOL_TABLE_REGISTRY
        key = version[:2]
    ...
    def _collect_data(self, processes):
        ...
        try:
            ...
            print("Cannot create process layer")
            return []
        ...
        if self.config.get("dump", False):
            self._dump_chains(int(task.UniqueProcessId), records)
