...
#   2) Cross-process: merge by NAME (child processes share modules
...
class Module_Extractor(interfaces.plugins.PluginInterface):
    ...
    def _build_children_map(self, all_tasks):
        """Build pid -> {child_pids} map from the full process list."""
        tasks_by_pid = {}
        children = {}
    ...
    def _detect_python_version(self, task):
        """
        Check VAD paths for pythonXY.dll.
        Returns (major, minor) or None if the process isn't Python.
        """
        return pe_parsing.detect_python_version_from_vads(self.context, task)
    ...
    def _collect_all(self):
        ...
        all_tasks = list(pslist.PsList.list_processes(
            self.context,
            self.config["kernel"],
        ))
        ...
        for root_pid in root_pids:
            if root_pid not in tasks_by_pid:
                print(f"WARNING: PID {root_pid} not found in process list")
                continue
        ...
        for t in unique_tasks:
            pid = int(t.UniqueProcessId)
            ppid = int(t.InheritedFromUniqueProcessId)
            comm = t.ImageFileName.cast(
                "string", max_length=t.ImageFileName.vol.count, errors="replace"
            )
            print(f"  PID {pid:6d}  PPID {ppid:6d}  {comm}")
        ...
        for task in unique_tasks:
            # Windows: verify process layer is accessible
            try:
            ...
            if not version:
                ...
                print(f"\n  PID {pid} ({comm}): not Python, skipping")
                continue
            ...
            print(f"\n{'='*60}")
            print(f"PID {pid} ({comm}) - Python {version[0]}.{version[1]}")
            print(f"{'='*60}")
            ...
            per_process[pid] = (comm, process_modules)

        # --- Phase 2: cross-process merge by name ---
        # Child processes have the same modules at different virtual addresses,
        # so we dedup by module name across all processes.
