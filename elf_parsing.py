"""
ELF Symbol Resolution for Volatility 3 — Memory-Only Parsing
...
Key insight: The kernel loads ELF content via PT_LOAD program headers. If the
...
them by:
  ...
  3. For static symbols like 'arenas': scanning the mapped read-only data
     segments for the .symtab/.strtab content directly by recognizing their
     structure

For statically-linked CPython (e.g., /usr/bin/python3.8), the approach is:
  - Parse PT_DYNAMIC → get .dynsym, .dynstr
  - If symbol not found there, scan the data segment for .symtab-like structures
  - Use the known structure of Elf64_Sym entries to validate candidates
...
def parse_elf_header(layer, base_address: int) -> Optional[Dict]:
    """Parse minimal ELF header. Returns dict or None."""
...
# ---------------------------------------------------------------------------
# Parse PT_LOAD segments — the memory map of what the kernel actually loaded
# ---------------------------------------------------------------------------

def parse_load_segments(layer, base_address: int, elf_info: Dict) -> List[Dict]:
    """
    Parse all PT_LOAD program headers to build the file-offset-to-vaddr map.
    
    This is crucial: the kernel maps ELF content according to PT_LOAD segments.
    If a file offset falls within a PT_LOAD's [p_offset, p_offset+p_filesz),
    then that data IS in memory at [p_vaddr + (file_offset - p_offset)].
    """
    segments = []
...
def file_offset_to_vaddr(segments: List[Dict], file_offset: int, is_pie: bool,
    ...
    """
    Convert an ELF file offset to a virtual address using PT_LOAD mappings.
    Returns None if the file offset is not within any loaded segment.
    """
    for seg in segments:
...
# ---------------------------------------------------------------------------
# Strategy 1: Section headers → symbol tables
# Works when section headers happen to fall within a PT_LOAD segment
# ---------------------------------------------------------------------------
...
def search_section_symbols(
    ...
    """
    Try to read section headers and resolve symbols through .symtab/.dynsym.
    
    Section headers are at e_shoff in the file. If that file offset is within
    a PT_LOAD segment, the section header table IS in memory and we can use it.
    """
    found = {}
    ...
    # Check if section headers are mapped into memory
    ...
    # Verify we can actually read from that address
    ...
    # Iterate section headers looking for SHT_SYMTAB (2) and SHT_DYNSYM (11)
    for i in range(e_shnum):
        ...
        # Map the symbol table file offset to memory
        ...
        # Read linked string table header
...
# ---------------------------------------------------------------------------
# Strategy 2: PT_DYNAMIC → .dynsym/.dynstr (always works for exported symbols)
# ---------------------------------------------------------------------------
...
def search_dynamic_symbols(
    ...
    """Search dynamic symbol table via PT_DYNAMIC."""
    found = {}
    remaining = set(symbol_names)
    is_64  = elf_info["is_64"]
    is_pie = elf_info["is_pie"]
    ...
    # DT_SYMTAB/DT_STRTAB values are already virtual addresses (not file offsets)
    # For ET_EXEC they're absolute; for ET_DYN they're relative to base
    if is_pie:
...
# ---------------------------------------------------------------------------
# Strategy 3: LTO-renamed variants (arenas.lto_priv)
# ---------------------------------------------------------------------------
...
def search_lto_symbols(
    ...
    """Search for .lto_priv suffixed variants."""
    lto_targets = set()
    original_map = {}
...
# ---------------------------------------------------------------------------
# Strategy 4: Scan mapped symtab-like data in PT_LOAD segments
# For when section headers are NOT in memory but .symtab data IS
# ---------------------------------------------------------------------------
...
def search_mapped_symtab(
    ...
    """
    Scan read-only PT_LOAD segments for .symtab/.strtab content.
    
    Even when section headers are stripped from memory, the actual symbol table
    and string table DATA may still be present if they fall within a PT_LOAD
    segment. We can find them by:
    
    1. Looking for a region of Elf64_Sym entries (24 bytes each) where:
       - st_name indices point into a nearby string table
       - st_value fields contain plausible virtual addresses
       - st_info fields have valid binding/type values
    
    2. The .strtab is usually right before or after .symtab and contains
       null-terminated ASCII strings
    
    This is the memory-only equivalent of reading section headers.
    """
    found = {}
    remaining = set(symbol_names)
    # Also search for LTO variants
    lto_map = {}
    for name in symbol_names:
    ...
    # Get the readable PT_LOAD segments (where .symtab data would be)
    readable_segments = []
    for seg in segments:
    ...
    # First, find candidate string tables by looking for regions dense with
    # null-terminated ASCII strings
    strtab_candidates = []
    for seg_vaddr, seg_size, seg_flags in readable_segments:
        # Skip executable segments (code, not data)
        if seg_flags & 1:  # PF_X
            continue
        ...
        # Scan for regions that look like string tables
        # A string table has a high density of printable ASCII + null bytes
        chunk_size = min(seg_size, 0x200000)
        data = _read_bytes(layer, seg_vaddr, chunk_size)
        if data is None:
            continue

        # Look for our target symbol names directly in the data
        for target in all_targets:
            ...
            while True:
                ...
                # Found the symbol name! Now figure out where the strtab starts.
                # Walk backwards to find the beginning of the string table
                # (string tables always start with a \x00 byte)
                strtab_start_idx = idx
                while strtab_start_idx > 0 and strtab_start_idx > idx - 0x100000:
                    # Check if this could be the start of strtab
                    if data[strtab_start_idx] == 0 and strtab_start_idx > 0:
                        # Verify the byte before is also printable or null
                        strtab_start_idx -= 1
                        continue
                ...
                # The actual start should be at a \x00 byte
                # Walk forward to find the first \x00
    ...
    # For each string table candidate, search nearby for a matching symtab
    for candidate in strtab_candidates:
        ...
        # The symtab is usually before the strtab in the file
        # Search backwards from the strtab for Elf64_Sym entries that
        # reference our string
        ...
        # Scan for Elf64_Sym entries where st_name == name_offset
        ...
        for offset in range(0, len(sym_data) - sym_size, sym_size):
            ...
            # Found a potential match! Extract the symbol value
            if is_64:
            ...
            # Validate: binding should be LOCAL(0), GLOBAL(1), or WEAK(2)
            if st_bind > 2:
                continue
            # Type should be NOTYPE(0), OBJECT(1), FUNC(2), SECTION(3), FILE(4)
            if st_type > 4:
                continue
            # Value should be a plausible address
            if st_value == 0:
                continue
            ...
            # Verify the resolved address is readable
...
# ---------------------------------------------------------------------------
# Strategy 5: Scan .bss for arena pointer pattern (structural recognition)
# ---------------------------------------------------------------------------
...
def scan_bss_for_arenas(
    ...
    """
    Scan writable segments for the arenas pointer by recognizing the
    arena_object structure pattern in memory.
    
    This is a memory-only approach: we scan the .data/.bss segments
    (which ARE always mapped) for a pointer that dereferences to a valid
    arena_object chain.
    """
    found = {}

    # Collect writable VMAs belonging to the Python binary
    python_rw_regions = []
    ...
    # Include adjacent anonymous mappings (BSS overflow)
    all_vmas = []
    for vma in task.mm.get_vma_iter():
    ...
    for region_start, region_end, region_path in python_rw_regions:
        ...
        for offset in range(0, scan_size - 8, 8):
            ...
            try:
                ...
                # Additional validation: check a second arena entry
...
def resolve_symbols(
    ...
    """
    Resolve symbols using memory-only ELF parsing.
    
    Strategies (in order):
      1. Section headers via PT_LOAD mapping
      2. PT_DYNAMIC → .dynsym (exported symbols)
      3. LTO variants (.lto_priv suffix)
      4. Mapped .symtab data scan (find symbol strings in loaded segments)
      5. .bss structural scan (arena pointer pattern recognition)
    """
    ...
    # Parse PT_LOAD segments — this is how we map file content to memory
    ...
    # Strategy 4: scan mapped data for symtab content
    found.update(search_mapped_symtab(layer, base_address, elf_info, remaining, segments))
    remaining = target_set - set(found.keys())
    if not remaining:
        return found

    # Strategy 5: .bss structural scan for arena pattern
...
def find_python_module_base(
    ...
    """Find the base address of a Python ELF in process memory."""
    lowest_base = None
    best_path = None
...
def find_symbol_in_process(
    ...
    """
    Primary API: find symbols in a Python module within a process.
    All parsing is done from memory only — no disk access.
    """
    layer = context.layers[proc_layer_name]
...
# ---------------------------------------------------------------------------
# Standalone plugin (optional CLI usage)
# ---------------------------------------------------------------------------
