...
class PyFrameObject(PyObject):
    ...
    def get_value(self, cur_depth=0, max_depth=10, visited=None):
        try:
            ...
            return f"<frame at {hex(self.vol.offset)}>"
...
class PyFunctionObject(PyObject):
    # 3.10: 128 bytes. func_globals moved to 16, func_builtins added at 24,
    # func_code moved to 48. Field access is by name via ISF so just make sure
    # the ISF offsets match. The code here is the same logic, just documenting
    # the new layout.
    @property
    def func_code_obj(self):
        return self._context.object(
